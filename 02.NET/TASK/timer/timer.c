...
#endif
#if GENERAL_TIM3
U32 tick_1ms = 0; // ms 计时变量
U8 tick_5ms = 0;  // 5ms计数器，作为主函数的基本周期
#endif
#if GENERAL_TIM4
U32 tick_1ms = 0; // ms 计时变量
U8 tick_5ms = 0;  // 5ms计数器，作为主函数的基本周期
...
static void GENERAL_TIM_Mode_Config(TimeModule time)
{
	...
	{
	case TIM_2:
		TIM_TimeBaseStructure.TIM_Period = GENERAL_TIM2_Period - 1; // 自动重装载寄存器的值，累计TIM_Period+1个频率后产生一个更新或者中断
		TIM_TimeBaseStructure.TIM_Prescaler = GENERAL_TIM2_Prescaler - 1;
		break; // 时钟预分频数
	case TIM_3:
		...
		break; // 时钟预分频数
	case TIM_4:
		TIM_TimeBaseStructure.TIM_Period = GENERAL_TIM4_Period - 1; // 自动重装载寄存器的值，累计TIM_Period+1个频率后产生一个更新或者中断
		TIM_TimeBaseStructure.TIM_Prescaler = GENERAL_TIM4_Prescaler - 1;
		break; // 时钟预分频数
	case TIM_5:
...
void TIM2_IRQHandler(void) // TIM3中断
{
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) // 检查TIM3更新中断发生与否
	{
		...
		if (time_2 % 30 == 0) // 30ms轮询一次
		{
			Check_Key_ON_OFF(); // 检测是否有按键按下
		}
		if (time_2 % 3000 == 0)
		{
			time_2 = 0;
...
void TIM5_IRQHandler(void) // TIM5中断
{
	if (TIM_GetITStatus(TIM5, TIM_IT_Update) != RESET) // 检查TIM3更新中断发生与否
	{
		...
		TIM_UPDATA++;								// 当回响信号很长时，记录溢出次数，每加一次代表加100us
